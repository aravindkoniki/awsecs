...
resource "aws_ecs_task_set" "ecs_task" {
  ...
  service         = try(aws_ecs_service.ecs[0].id, aws_ecs_service.ignore_task_definition[0].id)
  cluster         = var.cluster_arn
...
resource "aws_appautoscaling_target" "auto_scaling" {
  ...
  resource_id        = "service/${local.cluster_name}/${try(aws_ecs_service.ecs[0].name, aws_ecs_service.ignore_task_definition[0].name)}"
...
resource "aws_appautoscaling_policy" "auto_scaling_policy" {
  ...
  resource_id        = aws_appautoscaling_target.auto_scaling[0].resource_id
  scalable_dimension = aws_appautoscaling_target.auto_scaling[0].scalable_dimension
  service_namespace  = aws_appautoscaling_target.auto_scaling[0].service_namespace
...
resource "aws_appautoscaling_scheduled_action" "auto_scaling_policy_schedule" {

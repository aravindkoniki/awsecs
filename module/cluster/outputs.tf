...
output "cloudwatch_log_group_name" {
  ...
  value       = try(aws_cloudwatch_log_group.log[0].name, null)
}

output "cloudwatch_log_group_arn" {
  description = "ARN of CloudWatch log group created"
  value       = try(aws_cloudwatch_log_group.log[0].arn, null)
}
...
output "cluster_capacity_providers" {
  ...
  value       = { for k, v in aws_ecs_cluster_capacity_providers.cluster_capacity_providers : v.id => v }
}
...
output "autoscaling_capacity_providers" {
  ...
  value       = aws_ecs_capacity_provider.autoscaling_capacity_providers
}
